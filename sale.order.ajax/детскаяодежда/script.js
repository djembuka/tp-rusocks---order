window.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('#bx-soa-properties .bx-soa-customer-field').forEach((block) => {
    let control = block.querySelector('input, textarea');
    
    if (!control) return;
    
    if (control.value.trim() !== '') {
      block.classList.add('bx-soa-customer-field--active');
    }
    ...
    control.addEventListener('blur', () => {
      if (control.value.trim() !== '') {
        ...
        block.classList.remove('bx-soa-customer-field--active');
      }
    });
  });
});
...
(function () {
  ...
  function makeInputsActive() {
    //insert empty span
    let emptySpan = document.createElement('span');
    emptySpan.id = `BeforeSendRequestElement1`;
    document.querySelector('#bx-soa-properties .bx-soa-customer').appendChild(emptySpan);
    let counter = 0;
    //wait for the reload
    let intervalId = setInterval(() => {
      if (!document.getElementById(`BeforeSendRequestElement`)) {
        document.querySelectorAll('#bx-soa-properties .bx-soa-customer-field').forEach((block) => {
          let control = block.querySelector('input, textarea');
